...
- **AI 기능 확장**: 공통 생성 이력은 `ai_generations`에 저장합니다. 지원자 자동 평가는 범위 밖이며, 채용·매뉴얼 등으로 AI 기능을 넓힐 때도 기능별 이력 테이블을 먼저 늘리지 않고 이 공통 테이블을 확장하는 방향을 검토합니다.
...
- 현재 Alembic head: `20260814_0024_drop_notifications.py`
- Migration 파일 위치: `backend/migrations/versions/`
- Migration별 작업 배경과 진행 기록은 이 문서가 아니라 [`UPDATELOG.md`](../UPDATELOG.md)를 기준으로 확인합니다.
- 적용 명령: `cd backend && .\.venv\Scripts\alembic.exe upgrade head` (자세한 절차는 [`README.md`](../README.md#데이터베이스-준비) 참고)
