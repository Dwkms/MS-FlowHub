...
import { createManual, deleteManual, getManual, listManualCategories, updateManual } from "@/features/manuals/api";
...
export function ManualForm({ slug }: { slug?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try { if (slug) { await updateManual(slug, input); } else { await createManual(input); } router.replace("/manuals"); } catch (reason) { setError(reason instanceof Error ? reason.message : "매뉴얼 저장에 실패했습니다."); setSaving(false); }
  }

  async function removeManual() {
    if (!slug || !window.confirm("이 매뉴얼을 삭제할까요?")) return;
