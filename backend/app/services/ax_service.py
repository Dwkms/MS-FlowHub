"""AX 직원 도우미 — 매뉴얼·FAQ에서 답을 찾습니다.

**LLM을 호출하지 않습니다.** 등록된 문서에서 질문에 맞는 것을 찾아 원문을 보여줍니다.
사내 규정을 묻는 도구가 그럴듯한 문장을 지어내면 틀린 답이 규정처럼 읽히기 때문입니다.
문서에 없으면 "찾지 못했다"고 답합니다.

매칭 계산은 `app/domain/ax_search.py`, 답변 유형 판정은 `app/domain/ax_rules.py`에 있습니다.
이 Service는 그 둘을 순서대로 호출하고 로그를 남깁니다.

**역할별 공개 범위는 검색 후보 단계에서 거릅니다.** 답을 만든 뒤 가리는 게 아니라
애초에 후보에 넣지 않습니다.
"""
