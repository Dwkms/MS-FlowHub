"""채용 요청부터 지원자 관리까지의 업무 규칙.

흐름이 한 줄로 이어집니다.

    채용 요청 작성 → 전자결재로 상신 → 승인 → 공고 자동 생성 → 지원자 등록 → 전형 단계 관리

**공고는 사용자가 쓰는 문서가 아닙니다.** 결재가 승인되면 `process_approval`이 호출되고,
공고 본문은 `_build_posting_content`가 코드로 조립합니다. 승인된 사실을 그대로 옮기는 것이라
사람이 중간에 바꿔 쓸 여지를 두지 않았습니다.

지원자 전형은 `APPLIED → SCREENING → INTERVIEW → OFFERED → HIRED/REJECTED` 순이고,
`HIRED`와 `REJECTED`는 **되돌릴 수 없습니다.** 실수를 고치는 편의보다 기록의 신뢰를
택한 결정입니다.

이 파일이 가장 긴 Service인 이유는 요청·공고·지원자 세 가지를 한 흐름으로 다루기
때문입니다. 셋을 나누면 "승인되면 공고가 생긴다" 같은 연결 규칙이 어디에 있는지
찾기 어려워집니다.
"""
